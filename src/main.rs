...
fn run_mount(
    ...
    let mut options = vec![
        MountOption::FSName("clickfs".to_string()),
        MountOption::RO,
    ];
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async move {
            ...
            let _ = std::process::Command::new("fusermount")
                ...
                .arg(&mp_for_signal)
                .status();
            let _ = std::process::Command::new("umount").arg(&mp_for_signal).status();
        });
    });

    tracing::info!(mountpoint = %mountpoint.display(), "mounting");
    eprintln!("clickfs: mounted at {} (Ctrl-C to unmount)", mountpoint.display());
